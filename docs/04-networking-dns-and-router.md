...
This module documents the local network configuration required for the server and its clients.
...
Tailscale remote access and remote DNS configuration are covered in the next modules.
...
Replace example values with the values used by the environment. Use placeholders in published documentation and evidence.
...
A device can reach an IP address without being able to resolve hostnames. Network connectivity and DNS resolution must be tested separately.
...
The active interface should have an address in the local LAN range and a default route through the primary router.
...
- A reserved address outside the router's ordinary dynamic allocation range when possible.
...
A DHCP reservation is preferred over manually assigning an address on the server because address management remains centralized at the router.
...
Verify from the router's DHCP client or lease table that the reservation is active.
...
A failed ping does not always prove that routing is broken because some networks block ICMP. Use the routing table and application-level tests as additional evidence.
...
Before Pi-hole is active, the server and clients use the DNS service supplied by the router or another configured upstream resolver.
...
The router distributes the Pi-hole address to clients through DHCP. Clients should not need individual manual DNS settings when they use the primary router's DHCP service.

The server's own resolver configuration is handled separately because the server hosts Pi-hole. The final server DNS configuration should be applied only after Pi-hole is running and listening on port 53.
...
The resolver configuration must be consistent with the active network-management method. Avoid manually editing `/etc/resolv.conf` if it is managed by `systemd-resolved`, Netplan, NetworkManager, or systemd-networkd.

The final resolver path should be documented after Pi-hole is installed. Depending on the design, the server may use:
...
After Pi-hole is installed and verified, configure the primary router's LAN or DHCP DNS settings to provide:

```text
Primary DNS: <SERVER_LAN_IP>
```

Do not add a public DNS server as a secondary client DNS server when the requirement is for all client queries to pass through Pi-hole. A client may use the secondary resolver when the primary is slow or unavailable, bypassing filtering.

The router itself may use a separate upstream DNS configuration depending on its firmware and operating mode. The important client-side requirement is that DHCP distributes the Pi-hole address.
...
This step is performed after the Pi-hole module has deployed and verified the DNS service.
...
Configure the resolver so that its local stub does not compete with Pi-hole for port 53:
...
Ensure `/etc/resolv.conf` points to the resolver file intended by the selected design, then restart the resolver:
...
Pi-hole should be the intended listener for host port 53. If `systemd-resolved` still owns the local stub port, resolve that conflict before continuing.

The exact persistent resolver configuration belongs in the Pi-hole service module because it depends on the container being available.
...
The upstream gateway and primary router both perform routing and NAT. This creates a double-NAT environment.
...
Do not enable bridge mode or change the upstream gateway's operating mode unless the network requirements have been reviewed. The chosen mode may support other equipment or services that depend on the upstream gateway.
...
- The client is connected to the intended router rather than an isolated upstream network.
...
A blocked domain should return the response configured by Pi-hole, commonly `0.0.0.0` or another local blocked response.
...
Test both UDP and TCP DNS when investigating unusual failures. DNS commonly uses UDP, but TCP is required for some responses and fallback behavior.
...
- The DHCP reservation configuration without exposing identifying hardware details.
...
Do not publish MAC addresses, personal device names, router credentials, private hostnames, or unnecessary live addresses.
